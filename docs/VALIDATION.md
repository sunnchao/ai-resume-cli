...
| GitHub Windows / Linux / macOS 矩阵 | 核心 CI 矩阵是否全绿不以本机结果代替。本次只确认 Release 工作流的三平台二进制作业，见下行 |
| GitHub Release / 版本 tag 构建 | 已配置 `.github/workflows/release.yml`。本地 `actionlint` 1.7.12 通过；`tests/test_release_meta.py` 覆盖 tag 必须为 `vX.Y.Z`、与包装版本一致、平台压缩包命名和 SHA256SUMS。核心作业 `pytest -m "not ocr"` 154 passed。`workflow_dispatch` run [34356476374](https://github.com/sunnchao/ai-resume-cli/actions/runs/34356476374) 在 Ubuntu / macOS / Windows 构建并校验独立二进制成功；该次为非 tag 触发，GitHub Release 作业按设计跳过，不记录已发布 |
...
| GitHub Windows / Linux / macOS 独立二进制 | Release `workflow_dispatch` run [34356476374](https://github.com/sunnchao/ai-resume-cli/actions/runs/34356476374)：Ubuntu、macOS、Windows 均 `build_binary.py` + `verify_binary.py` 通过。Windows 校验输出 `PASS --version/parse/extract/score/batch` 与 `OK ...\resume-cli.exe` |
